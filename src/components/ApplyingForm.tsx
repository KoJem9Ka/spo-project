...
const ApplyingForm: FC<TFromState> = ( {
  isRu,
  lastName,
  firstName,
  patronymic,
    ...
    <>
      ...
      {!isRu ? closestOffice : <>
        ...
        <div>
          ...
          <div className='flex gap-8'>
            <button className={join( 'radioBtn', gender === 'female' && 'active' )} onClick={() => formChange( { gender: 'female' } )}>Женский</button>
            <button className={join( 'radioBtn', gender === 'male' && 'active' )} onClick={() => formChange( { gender: 'male' } )}>Мужской</button>
          </div>
        </div>
        <FormField type='date'
                   required
